...
function pages($meta_key='') { // $meta_key has something to do with Gregs custom fields mod
	...
	echo ereg_replace("\"><a [/\?a-zA-Z0-9\-\.\:\"\=\_ >]+</a>([\t\n]+)<ul"," haschildren\\0",wp_list_pages('title_li=&'.$suckerfish_pageorderecho.get_option('suckerfish_includeexcludepages').'='. get_option('suckerfish_excludepages').'&echo=0'.$suckerfish_depthpagesecho.'&meta_key='.$meta_key)) , "";
	// For putting the pages into a sinle dropdown
	if (get_option('suckerfish_pages_singledropdown') == 'on') {
		echo '</ul></li>';
	}
