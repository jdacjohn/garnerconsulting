...
function pixopoint_menu_header() {global $pixopoint_codingengine;
	if (!is_admin() AND current_user_can('manage_options')) {
		...
		else {echo '<link rel="stylesheet" href="'.$pixopoint_codingengine.'style.php?id='.get_option('pixo_saved').'" media="screen" />';}
		?>
...
function pixopoint_menu_head() {global $variable,$pixopoint_menu_version,$pixopoint_codingengine;
	...
	ob_start();
	if (isset($_POST['open'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {echo get_option('suckerfish_save');} // load saved design

	// Loads the correct data when using the 'save' tool
	elseif ($variable['designsaved'] == 'yes') {echo get_option('suckerfish_save');} // Load first menu
	...
	// Load Nature
	elseif (isset($_POST['nature'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Nature';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/nature.php');} // Load first menu
	elseif (isset($_POST['nature2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Nature';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/nature2.php');} // Load second menu

	// Load Bland grey
	elseif (isset($_POST['blandgrey'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Bland Grey';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blandgrey.php');} // Load first menu
	elseif (isset($_POST['blandgrey2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Bland Grey';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blandgrey2.php');} // Load second menu

	// Load Blue berry
	elseif (isset($_POST['blueberry'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Blue berry';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blueberry.php');} // Load first menu
	elseif (isset($_POST['blueberry2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Blue berry';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blueberry2.php');} // Load second menu

	// Load Bland Red
	elseif (isset($_POST['blandred'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Bland Red';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blandred.php');} // Load first menu
	elseif (isset($_POST['blandred2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Bland Red';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/blandred2.php');} // Load second menu

	// Load Red Dazzle
	elseif (isset($_POST['reddazzle'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Red Dazzle';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/reddazzle.php');} // Load first menu
	elseif (isset($_POST['reddazzle2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Red Dazzle';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/reddazzle2.php');} // Load second menu

	// Load Corporate Black
	elseif (isset($_POST['corporateblack'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Corporate Black';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/corporateblack.php');} // Load first menu
	elseif (isset($_POST['corporateblack2'])&& wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {$variable['confirmloading'] = 'Corporate Black';readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));require('designs/corporateblack2.php');} // Load second menu

	elseif (get_option('pixo_saved') == 'new') {echo get_option('suckerfish_save');} // If new then load default template
	elseif (wp_verify_nonce($_REQUEST['_wpnonce'], 'pxp_form')) {readfile($pixopoint_codingengine.'data.php?id='.get_option('pixo_saved'));} // else load from external server
